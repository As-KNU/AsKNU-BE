    ...
    for it in items:
        try:
            ...
            if not content or len(content) < 30:
                skipped += 1
                if len(errors) < 5: errors.append({"url": it.get("url"), "error": "content_too_short"})
                continue
            ...
            skipped += 1
            if len(errors) < 5: errors.append({"url": it.get("url"), "error": repr(e)})

    return {"status": "ok", "saved": saved, "skipped": skipped, "count": len(items), "sample_errors": errors}
...
async def chat(payload: ChatRequest, years: int = 3):
    ...
    if not rows:
        return {"answer": "관련 공지를 찾지 못했어요. 키워드를 바꿔보거나 담당자에게 문의하세요.", "citations": []}

    answer = await answer_with_gemini(payload.question, rows)
    citations = [{"title": r["title"], "url": r["url"], "posted_at": r["posted_at"], "summary": r.get("summary") or ""} for r in rows]
